...
function showPartDetail(p) {
  ...
  document.getElementById('part-detail').innerHTML = `
    <div class="detail-header">
      <span style="font-size:2rem">${PART_ICONS[p.id] || '🔩'}</span>
      <h2>${p.name}</h2>
      <span class="category-tag">${CATEGORIES[p.category] || p.category}</span>
    </div>
...
function addPartToBuild(id) {
  ...
  if (!part || selectedParts.find(p => p.id === id)) return;
...
function renderBuilderView(mode) {
  ...
  if (mode === 'wiring') {
    ...
    if (hasController) {
      ...
      code += '  delay(100);\n}\n';
    } else code += '// Add a microcontroller to generate code.\n';
    canvas.innerHTML = `<h3 style="margin-bottom:0.75rem;color:var(--primary)">Generated Starter Code</h3>
      <pre style="background:var(--bg);padding:1rem;border-radius:8px;overflow:auto;font-size:0.85rem;line-height:1.5;font-family:'JetBrains Mono',monospace;border:1px solid var(--border)">${code}</pre>`;
  }
}

function validateBuild() {
  const notes = document.getElementById('compatibility-notes');
  if (selectedParts.length === 0) { notes.innerHTML = '<p style="color:var(--text-muted)">Add some parts first.</p>'; return; }
  const issues = [];
...
function initLessons() {
  ...
  grid.innerHTML = MODULES.map(m => `<div class="module-card" data-id="${m.id}"><h3>${m.title}</h3><p>${m.desc}</p></div>`).join('');
  grid.querySelectorAll('.module-card').forEach(card => {
    card.addEventListener('click', () => {
      const mod = MODULES.find(m => m.id === card.dataset.id);
      const content = document.getElementById('lesson-content');
      content.style.display = 'block';
      content.innerHTML = `<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-top:1.5rem">
        ...
        <button class="secondary-btn" style="margin-top:1rem" onclick="document.getElementById('lesson-content').style.display='none'">Close</button></div>`;
...
function calcBattery() {
  ...
  document.getElementById('batt-result').textContent = `≈ ${(cap/draw).toFixed(1)} hours`; addXP(3);
}
function calcGear() {
